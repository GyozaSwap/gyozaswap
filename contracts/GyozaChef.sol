...
import "./GyozaToken.sol";


interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to GyozaSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // GyozaSwap must mint EXACTLY the same amount of GyozaSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// GyozaChef is the master of Gyoza. He can make Gyoza and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GYOZA is sufficiently
...
contract GyozaChef is Ownable {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accGyozaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    constructor(
        GyozaToken _gyoza,
        address _devaddr,
        uint256 _gyozaPerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        gyoza = _gyoza;
        devaddr = _devaddr;
        gyozaPerBlock = _gyozaPerBlock;
    ...
    function pendingGyoza(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 gyozaReward = multiplier.mul(gyozaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accGyozaPerShare = accGyozaPerShare.add(gyozaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accGyozaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Withdraw LP tokens from GyozeChef.
