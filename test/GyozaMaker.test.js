...
const GyozaMaker = artifacts.require('GyozaMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('GyozaMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.gyoza = await GyozaToken.new({ from: alice });
        await this.gyoza.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await GyozaMaker.new(this.factory.address, bar, this.gyoza.address, this.weth.address);
        this.gyozaWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.gyoza.address)).logs[0].args.pair);
    ...
    it('should make GYOZAs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.gyozaWETH.address, '10000000', { from: minter });
        await this.gyoza.transfer(this.gyozaWETH.address, '10000000', { from: minter });
        await this.gyozaWETH.mint(minter);
        ...
        // After calling convert, bar should have GYOZA value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.gyoza.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for GYOZA-ETH pair
        await this.gyoza.transfer(this.gyozaWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.gyozaWETH.address, '100000', { from: minter });
        await this.gyozaWETH.sync();
        await this.gyoza.transfer(this.gyozaWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.gyozaWETH.address, '10000000', { from: minter });
        await this.gyozaWETH.mint(minter);
        assert.equal((await this.gyozaWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.gyoza.address, this.weth.address);
        assert.equal((await this.gyoza.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.gyozaWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});